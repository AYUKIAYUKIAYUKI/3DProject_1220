...
#define AMOUNT_NUM (18)	//使える数字の最大数
...
void UpdateRankingDesign(void)
{
	for (int i = 0; i < AMOUNT_NUM; i++)
	{
		PrintDebugProc("pos:%f %f %f\n", g_aRankingScore[i].pos.x, g_aRankingScore[i].pos.y, g_aRankingScore[i].pos.z);
	}
...
//構造体の情報を設定する関数
//************************************************
void SetInfoRankingDesign(int mode)
{
	//デザイン
	for (int i = 0; i < TexMax; i++)
	{
